...
function renderHistorial() {
  ...
  // Si no hay detalle pero sí hay totales de caja de ese día, es que el
  // detalle ya se archivó solo (ver HISTORIAL_DETALLE_RETENCION_DIAS) —
  // no es que ese día no hubiera ventas, así que el aviso lo deja claro en
  // vez de parecer que se han perdido datos.
  const detalleArchivado = list.length === 0 && !esHoy && localStorage.getItem(getCajaTotalesKey(historialFechaSel)) !== null;
  el.innerHTML = filtrado.length === 0
    ? `<div class="historial-empty">${detalleArchivado ? 'El detalle de este día ya se archivó (se guarda ' + HISTORIAL_DETALLE_RETENCION_DIAS + ' días). Los totales siguen exactos en "🧮 Hacer caja".' : (list.length === 0 ? 'No hay comandas guardadas ese día.' : 'Ningún pedido coincide con la búsqueda.')}</div>`
    : filtrado.map(([o, i]) => {
