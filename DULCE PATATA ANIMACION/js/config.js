...
(function() {
  ...
  window.fb_decrementSlot = async function(s) { await db.ref("slots/"+tK()+"/"+s).transaction(function(v){return Math.max(0,(v||0)-1);}); };
  ...
  window.fb_resetOrderStatuses = async function() { await jset("orderStatus/"+tK(),null); };
  // ANTI-SPAM
  window.fb_saveBlacklist     = async function(l) { await jset("config/blacklist", jstr(l)); };
  window.fb_loadBlacklist     = async function()  { var sn=await jget("config/blacklist"); return sn.exists()?jparse(sn.val()):null; };
  window.fb_saveAntiSpamCfg   = async function(c) { await jset("config/antiSpamCfg", jstr(c)); };
  window.fb_loadAntiSpamCfg   = async function()  { var sn=await jget("config/antiSpamCfg"); return sn.exists()?jparse(sn.val()):null; };
  window.fb_loadPhoneLog      = async function(d) { var sn=await jget("phoneLog/"+d); return sn.exists()?sn.val():null; };
  window.fb_updatePhoneLog    = async function(d,phone) {
    await db.ref("phoneLog/"+d+"/"+phone).transaction(function(entry) {
      if (!entry) return { count: 1, lastTs: Date.now() };
      return { count: (entry.count||0)+1, lastTs: Date.now() };
    });
  };
  // BANNER DEL DÍA
  window.fb_saveBannerDia  = async function(d) { await jset("config/bannerDia", jstr(d)); };
  window.fb_listenBannerDia = function(cb) { return jlisten("config/bannerDia", function(sn){ cb(sn.exists()?jparse(sn.val()):null); }); };
  // OPEN
