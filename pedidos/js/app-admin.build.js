...
function toggleSoldout(id) {
  ...
  // Al volver a marcarlo disponible a mano, se olvida el "permanente" de la
  // vez anterior — si no, la próxima vez que se agote este producto por un
  // motivo normal (se acabó la masa de hoy) saldría ya marcado como
  // permanente sin que nadie lo haya pedido esta vez.
  if (!item.soldout) item.soldoutPermanente = false;
  saveMenu();
  renderMenu();
  renderAdminProducts();
  showToast('prod-toast');
...
// "Agotado permanente" — evita que el reseteo automático de medianoche
// (scheduleSlotMidnightReset, nucleo-compartido.js) vuelva a poner
// disponible un producto que se ha quitado a propósito (fuera de
// temporada, descatalogado...), no solo porque hoy se acabó. Sin esto,
// cualquier producto agotado volvía disponible solo cada noche sin
// distinción.
function toggleSoldoutPermanente(id) {
  const item = MENU.find(m => m.id == id);
  if (!item) return;
  item.soldoutPermanente = !item.soldoutPermanente;
  saveMenu();
  logActivity((item.soldoutPermanente ? '🔒' : '🔄') + ' Producto "' + item.name + '" marcado ' + (item.soldoutPermanente ? 'agotado permanente' : 'para reponerse solo cada día'));
}
